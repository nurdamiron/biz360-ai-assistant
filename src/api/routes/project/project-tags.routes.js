// src/api/routes/project/project-tags.routes.js
...
router.get('/', authenticateCombined, async (req, res) => {
  try {
    ...
    logger.error(`Ошибка при получении тегов проекта #${req.params.id}:`, error);
    res.status(500).json({ 
      success: false,
...
router.post('/', authenticateCombined, async (req, res) => {
  try {
    ...
    logger.error(`Ошибка при добавлении тега к проекту #${req.params.id}:`, error);
    res.status(500).json({ 
      success: false,
...
/**
 * @route   DELETE /api/projects/:id/tags/:tagName
...
router.delete('/:tagName', authenticateCombined, async (req, res) => {
  try {
    ...
    logger.error(`Ошибка при удалении тега у проекта #${req.params.id}:`, error);
    res.status(500).json({ 
      success: false,
...
/**
 * @route   GET /api/projects/:id/tags/available
