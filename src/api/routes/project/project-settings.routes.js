// src/api/routes/project/project-settings.routes.js
...
/**
 * @route   GET /api/projects/:id/settings
...
router.get('/', authenticateCombined, async (req, res) => {
  try {
    ...
    logger.error(`Ошибка при получении настроек проекта #${req.params.id}:`, error);
    res.status(500).json({ 
      success: false,
...
/**
 * @route   GET /api/projects/:id/settings/:key
...
router.get('/:key', authenticateCombined, async (req, res) => {
  try {
    ...
    logger.error(`Ошибка при получении настройки проекта #${req.params.id}:`, error);
    res.status(500).json({ 
      success: false,
...
/**
 * @route   PUT /api/projects/:id/settings/:key
...
router.put('/:key', authenticateCombined, async (req, res) => {
  try {
    ...
    logger.error(`Ошибка при обновлении настройки проекта #${req.params.id}:`, error);
    res.status(500).json({ 
      success: false,
...
/**
 * @route   DELETE /api/projects/:id/settings/:key
...
router.delete('/:key', authenticateCombined, async (req, res) => {
  try {
    ...
    logger.error(`Ошибка при удалении настройки проекта #${req.params.id}:`, error);
    res.status(500).json({ 
      success: false,
...
/**
 * @route   POST /api/projects/:id/settings/reset
...
router.post('/reset', authenticateCombined, async (req, res) => {
  try {
    ...
    logger.error(`Ошибка при сбросе настроек проекта #${req.params.id}:`, error);
    res.status(500).json({ 
      success: false,
